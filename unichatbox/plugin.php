<?php
namespace Gener8Tor\license;
class wpgdprpluginlisence
{
	protected $plugin;
	protected $prefix;
  ...
  return $result;
	...
	function reValidate($type="frontend")
	{
		...
		function revqlidte".$this->plugin."()
		{
			...
			srvr.send('action=gener8toractionresponse_adminajxlcnc&reverifyjkmvhblicense=1&rvryfyplugin=".$this->plugin."&rvryfypluginpref=".$this->prefix."&pluginnonce=".$plugincsrf."');
		}
...
class gdpractivationpage extends  wpgdprpluginlisence
{
	function __construct($arr)
	{
	...
	function loadActivationPage()
	{
		...
		add_menu_page('WP_Gener8tor', 'WP Gener8tor', 'administrator', 'uni_settings_page',array($this,$callback),'');
	}
